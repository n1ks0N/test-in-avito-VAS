		...
		case 'SELECT-CHANGE':
			if (state[0] !== action.size) {
				...
				for (let i = 1; i < state.length; i++) {
					// сортировка от меньшего к большему по первому числу для возврата предыдущего выбора на правильную позицию
					if (state[i].split(' ')[0] > beforeSelected.split(' ')[0]) {
						state.splice(i, 0, beforeSelected); // запись предыдущего выбора на правильную позицию
						break;
					} else if (state[i].split(' ')[0] === beforeSelected.split(' ')[0]) {
						// если первые числа равны проверка по второму числу
					...
					if (i === state.length - 1) {
						// если предыдущий выбор оказалася наибольшим
...
export const selectActionCreater = (size) => ({
	type: 'SELECT-CHANGE',
	size: size
});
